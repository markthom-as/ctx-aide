...
2. Build a review packet with `ctxa feedback review --repo <repo> --ticket <ticket> --screenshot <path> --url <url> --json`.
...
4. For visual review across many screenshots, opt into the beta UI through onboarding with `ctxa adoption bootstrap --repo <repo> --enable-screenshot-feedback-ui --write --json` or later settings with `ctxa settings set --repo <repo> --feature screenshot-feedback-review-ui --enabled true --write --json`.
5. Start the local UI with `ctxa feedback review-ui --repo <repo> --screenshot-dir .ctx-aide/artifacts/screenshots --port 0`.
...
8. Plan a natural-language feedback response with `ctxa feedback plan --repo <repo> --ticket <ticket> --body "<feedback>" --json` when working in the JSON-only path.
...
12. Capture operator feedback with `ctxa feedback capture --repo <repo> --ticket <ticket> --title "<title>" --body "<feedback>" --screenshot <path> --url <url> --write --json`.
13. Promote clear feedback with `ctxa feedback promote --mode acceptance-criteria` when it should tighten the current ticket.
14. Promote separate work with `ctxa feedback promote --mode follow-up-ticket` when it should become a new atomic ticket.
...
- `ctxa feedback review-ui` is local-only, binds to `127.0.0.1`, remains optional beta setup, and has an expected cost delta of `$0/month`.
...
- `ctxa feedback review --repo <repo> --ticket <ticket> --screenshot <path> --url <url> --json`
- `ctxa settings get --repo <repo> --json`
- `ctxa settings set --repo <repo> --feature screenshot-feedback-review-ui --enabled true --write --json`
- `ctxa feedback review-ui --repo <repo> --screenshot-dir .ctx-aide/artifacts/screenshots --port 0`
- `ctxa feedback review-ui --repo <repo> --plan-only --json`
- `ctxa feedback plan --repo <repo> --ticket <ticket> --body "<natural feedback>" --json`
- `ctxa feedback capture --repo <repo> --ticket <ticket> --title "<title>" --body "<feedback>" --write --json`
- `ctxa feedback promote --repo <repo> --feedback <feedback-id-or-path> --ticket <ticket> --mode acceptance-criteria --write --json`
- `ctxa feedback promote --repo <repo> --feedback <feedback-id-or-path> --ticket <ticket> --mode follow-up-ticket --write --json`
