...
2. Build a review packet with `ctx-aide feedback review --repo <repo> --ticket <ticket> --screenshot <path> --url <url> --json`.
...
4. For visual review across many screenshots, opt into the beta UI through onboarding with `ctx-aide adoption bootstrap --repo <repo> --enable-screenshot-feedback-ui --write --json` or later settings with `ctx-aide settings set --repo <repo> --feature screenshot-feedback-review-ui --enabled true --write --json`.
5. Start the local UI with `ctx-aide feedback review-ui --repo <repo> --screenshot-dir .ctx-aide/artifacts/screenshots --port 0`.
...
8. Plan a natural-language feedback response with `ctx-aide feedback plan --repo <repo> --ticket <ticket> --body "<feedback>" --json` when working in the JSON-only path.
...
12. Capture operator feedback with `ctx-aide feedback capture --repo <repo> --ticket <ticket> --title "<title>" --body "<feedback>" --screenshot <path> --url <url> --write --json`.
13. Promote clear feedback with `ctx-aide feedback promote --mode acceptance-criteria` when it should tighten the current ticket.
14. Promote separate work with `ctx-aide feedback promote --mode follow-up-ticket` when it should become a new atomic ticket.
...
- The review UI is a beta feature and must be disabled by default in `docs/config/ctx-aide.settings.json`.
...
- `.ctx-aide/artifacts/` remains a local artifact area unless a target repo explicitly chooses to commit selected evidence.
- `ctx-aide feedback review-ui` is local-only, binds to `127.0.0.1`, remains optional beta setup, and has an expected cost delta of `$0/month`.
...
- `ctx-aide feedback review --repo <repo> --ticket <ticket> --screenshot <path> --url <url> --json`
- `ctx-aide settings get --repo <repo> --json`
- `ctx-aide settings set --repo <repo> --feature screenshot-feedback-review-ui --enabled true --write --json`
- `ctx-aide feedback review-ui --repo <repo> --screenshot-dir .ctx-aide/artifacts/screenshots --port 0`
- `ctx-aide feedback review-ui --repo <repo> --plan-only --json`
- `ctx-aide feedback plan --repo <repo> --ticket <ticket> --body "<natural feedback>" --json`
- `ctx-aide feedback capture --repo <repo> --ticket <ticket> --title "<title>" --body "<feedback>" --write --json`
- `ctx-aide feedback promote --repo <repo> --feedback <feedback-id-or-path> --ticket <ticket> --mode acceptance-criteria --write --json`
- `ctx-aide feedback promote --repo <repo> --feedback <feedback-id-or-path> --ticket <ticket> --mode follow-up-ticket --write --json`
