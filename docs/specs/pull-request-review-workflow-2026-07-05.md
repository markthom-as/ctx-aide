...
- Routes: none.
- Files/directories: `tools/context/ctx.mjs`, `docs/workflows/`, `docs/config/repo-context.tools.json`, `docs/specs/`, `docs/tickets/`, `docs/ticket-packs/`, and `README.md`.
- Components: none.
- Flows: repo-context dogfood, ticket completion, feedback review.
- Design-system areas: none.

## Existing Context

- Workflow markdown files are validated by `ctx lint`.
...
- Rationale: `ctx workflow deps` should prove whether the local command surface exists before an agent attempts PR work.
...
- Shared files that require coordination: `tools/context/ctx.mjs`, `docs/config/repo-context.tools.json`, `README.md`, and workflow docs.
